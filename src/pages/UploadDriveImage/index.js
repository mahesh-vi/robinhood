...
export default class UploadDriveImage extends Component {
  ...
  upload() {
    ...
    if (album_images.length > 0) {
      ...
      DriveService.uploadImageInDrive(id || drive_id, album_images)
        .then((res) => {
          ...
          <View style={{flexDirection: 'row', marginVertical: 10}}>
            ...
            <View style={{marginLeft: 20}}>
              <Text style={[StyleApp.headerText]}>
                {"Upload Drive's Image"}
              </Text>
              <Text style={[StyleApp.headerSubText]}>
                {'Upload drive images here'}
              </Text>
            </View>
          </View>
          ...
          <TouchableOpacity style={{marginVertical: 5}} onPress={this.getImage}>
            <View
              ...
              <Text
                ...
                Upload Photos of the revent drive which reflects food
                distribution.
              </Text>
            </View>
          </TouchableOpacity>
          {this.state.error && (
            <Text style={[AppStyle.errorText]}> {this.state.error}</Text>
          )}
