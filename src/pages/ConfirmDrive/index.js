...
export default class CreateDrive extends Component {
    ...
    componentDidMount() {
        ...
        console.log(Object.assign(this.state.driveDetail, { date: date + (drive_time ? " " + drive_time : "") }))
        this.setState({
            driveDetail: Object.assign(this.state.driveDetail, { date: date + (drive_time ? " " + drive_time : "") })
        });
        Promise.all([
            UserService.getInviteUserList(),
            CommonService.getFoodType(),
        ]).then((res) => {
    ...
    createDrive = () => {
        const { inviteList, foodTypes, image_uri, doner_id, query, album_images } = this.state;
        ...
        if (inviteList && foodTypes) {
            ...
            if ((Object.keys(this.validateField()).length == 0)) {
                ...
                CityAdminService.confirmDrive(this.state.driveDetail, selectedInvite, selectedFoodType, poc, doner_id, query, album_images)
                    .then((res) => {
                        console.log(res);
                        ...
                        const message = this.state.driveDetail.name + ((this.state.driveDetail.poc) ? ' drive edit successfully' : ' drive confirm successfully.');
  ...
  */
    ...
    */
    validateField() {
        const { name, donar_name, description, no_of_volunteers, food_quality, pickup_address, date } = this.state.driveDetail;
    ...
    renderInvites = (item) => {
        ...
        return (<Menu key={item.id}>
            ...
            <MenuOptions customStyles={{ optionText: { padding: 0, fontFamily: 'Montserrat-Medium', margin: 0 } }}>
                {item.id != global.user.id && <MenuOption onSelect={() => this.removeRobin(item)} text={'Remove'} />}
                {(!item.poc && global.user.id != this.state.driveDetail.poc) && <MenuOption onSelect={() => this.makePOC(item)} text='Make POC' />}
            </MenuOptions>
        </Menu>);
    ...
    makePOC = (robin) => {
        ...
        _this.setState({ inviteList: inviteList, driveDetail: Object.assign(this.state.driveDetail, { poc: robin.id }) });
    }
    ...
    render() {
        const imageURL = this.state.image_uri ? { uri: this.state.image_uri.uri } : this.state.driveDetail.image_url ? { uri: this.state.driveDetail.image_url } : null;
        ...
        return (
            <ScrollView style={styles.mainContainer}>
                ...
                <View style={{ marginHorizontal: 20 }}>
                    ...
                    {imageURL && <Image source={imageURL} style={[{ width: 80, height: 80, borderRadius: 40 }]} />}
                    <FlatList
                    ...
                    <TouchableOpacity style={[{ marginVertical: 10, height: 30, alignItems: 'center', justifyContent: 'center' }, AppStyle.primaryBackgroundColor]}
                        onPress={() => this.getImage()}>
                        <Text style={{ color: '#fff', textTransform: 'uppercase', fontFamily: 'Montserrat-Regular' }}>Drive Images</Text>
                    </TouchableOpacity>
                </View>
                ...
                <View style={{ marginLeft: 20 }}>
                    <ExtendDatePicker label="Choose date" mode="datetime" date={new Date(this.state.driveDetail.date)}
                        onSelect={(date) => {
                    ...
                    <Autocomplete
                        ...
                        renderItem={({ item }) => (
                            ...
                            <TouchableOpacity key={"" + item.id + ""} onPress={() => {
                                this.setState({ doner_id: item.id, query: item.firstname + " " + item.lastname });
                ...
                <View style={{ margin: 20 }}>
                    ...
                    <TextInput
                        style={[{ borderBottomWidth: 1 / 2, borderColor: 'rgba(0,0,0,0.5)', padding: 0, maxHeight: 50 }, AppStyle.regularFont]}
                        multiline={true}
                        onChangeText={(text) => this.setState({ driveDetail: Object.assign(this.state.driveDetail, { pickup_address: text }) })}
                ...
                <View style={{ margin: 20, flexDirection: 'row' }}>
                    <View style={{ flex: 1 }}>
                        ...
                        <TextInput
                            ...
                            onChangeText={(text) => this.setState({ driveDetail: Object.assign(this.state.driveDetail, { food_quality: text }) })}
                            value={this.state.driveDetail.food_quality} />
                        {this.state.formErrors.food_quality && <Text style={[AppStyle.errorText]}> {this.state.formErrors.food_quality}</Text>}
                    </View>
                    ...
                    <View style={{ flex: 1, marginLeft: 5 }}>
                        ...
                        <TextInput
                            ...
                            placeholder={'Min 3+'}
                            style={[{ borderBottomWidth: 0.5, borderColor: 'rgba(0,0,0,0.5)', padding: 0, maxHeight: 50 }, AppStyle.regularFont]}
                            onChangeText={(text) => this.setState({ driveDetail: Object.assign(this.state.driveDetail, { no_of_volunteers: text }) })}
                            value={this.state.driveDetail.no_of_volunteers.toString()} />
                        {this.state.formErrors.no_of_volunteers && <Text style={[AppStyle.errorText]}> {this.state.formErrors.no_of_volunteers}</Text>}
                ...
                <View style={{ marginHorizontal: 20, marginTop: 10 }}>
                    ...
                    {this.state.formErrors.general && <Text style={[AppStyle.errorText]}> {this.state.formErrors.general}</Text>}
                </View>
                ...
                >
                    <Text style={{ color: '#fff', textTransform: 'uppercase', fontFamily: 'Montserrat-Bold' }}>{this.state.driveDetail.poc ? 'Edit Drive' : 'Confirm DRIVE'}</Text>
                </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    inputContainer: {
        ...
        backgroundColor: 'rgba(250,250,254,1)', marginBottom: 10, width: '100%', fontFamily: 'Montserrat-Regular', fontSize: 16
    },
    avtarImage: {
        width: 40,
        height: 40,
