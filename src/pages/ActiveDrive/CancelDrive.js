...
export default class CancelDrive extends Component {
    ...
    onCancel = () => {
        if ((Object.keys(this.validateField()).length == 0)) {
            ...
            DriveService.cancelDrive(id || drive_id, selectedReason, cancel_message).then((res) => {
                console.log(res);
    ...
    validateField() {
        ...
        if (!selectedReason) {
            ...
            if (selectedReason == this.state.cancelReaons.other) {
                if (!cancel_message) {
                    fieldValidationErrors.cancel_message = 'Message required for Other reason';
                }
            }
        }
                    ...
                    <ScrollView style={{ backgroundColor: "#fff" }} >
                        <View style={[styles.mainView, AppStyle.boxWithShadow]}>
                            ...
                            <View style={{ marginHorizontal: 20 }}>
                                <Text style={[AppStyle.mediumFont, AppStyle.blackColor, StyleApp.font24]}>{'Are you sure you want to cancel this drive?'}</Text>
                                ...
                                <View style={{ marginVertical: 10, }}>
                                    <Text style={[AppStyle.text14, AppStyle.mediumFont]}>Please select a reason <Text style={[{ color: '#f00' }]}>*</Text></Text>
                                    ...
                                    {this.state.formErrors.selectedReason && <Text style={[AppStyle.errorText]}> {this.state.formErrors.selectedReason}</Text>}

                                </View>

                                {(this.state.cancelReaons && this.state.selectedReason && this.state.selectedReason == this.state.cancelReaons.other) &&
                                ...
                                <View>
                                    ...
                                    <TouchableOpacity style={[{ marginTop: 10, height: 44, paddingHorizontal: 30, borderRadius: 22, alignItems: 'center', justifyContent: 'center' }, AppStyle.primaryBackgroundColor
                                        ...
                                        <Text style={{ color: '#fff', textTransform: 'uppercase', fontFamily: 'Montserrat-Bold' }}>Cancel Drive</Text>
                                    </TouchableOpacity>
