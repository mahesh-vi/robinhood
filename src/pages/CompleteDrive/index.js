    ...
    */
    ...
    submit = () => {
        ...
        try {
            if ((Object.keys(this.validateField()).length == 0)) {
                ...
                const { total_robins, food_quantity, album_images, description, count_serve } = this.state;
                ...
                DriveService.completeDrive(id || drive_id, requestData).then((res) => {
    ...
    validateField() {
        ...
        if (!food_quantity) {
            ...
            fieldValidationErrors.food_quantity = 'Food Quantity must be greater than 0';
        }

        if (!count_serve) {
            fieldValidationErrors.count_serve = 'Served People is required';
        } else if (count_serve <= 0) {
            fieldValidationErrors.count_serve = 'Served People must be greater than 0';
        }


        if (album_images.length < 4) {
            fieldValidationErrors.album_images = 'Upload minimum 4 Photos is required';
        }
            ...
            <ScrollView style={styles.mainContainer}>
                ...
                <View style={{ marginVertical: 15 }}>
                    ...
                    <View style={{ marginHorizontal: 30, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <FeatherIcon size={25} name={'x'} color={AppStyle.grayColor.color} onPress={() => this.props.navigation.goBack()} />
                    </View>
                    ...
                    <View style={[{ padding: 20, borderWidth: 0, margin: 10 }]}>
                        ...
                        <View style={{ flex: 1, marginVertical: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Text style={[StyleApp.regularFont, StyleApp.font12, { flex: 0.6, flexWrap: 'wrap' }]}>Specify the overall Food Quantity</Text>
                            <TextInput
                                blurOnSubmit
                                returnKeyType='done'
                                style={[AppStyle.boldFamily, AppStyle.text20, { borderBottomWidth: 2, width: 60, textAlign: 'center' }]}
                                value={this.state.food_quantity}
                                keyboardType={'number-pad'}
                                onChangeText={(text) => this.setState({ food_quantity: text })}
                            />
                        </View>
                        {this.state.formErrors.food_quantity && <Text style={[AppStyle.errorText]}> {this.state.formErrors.food_quantity}</Text>}


                        <View style={{ flex: 1, marginVertical: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Text style={[StyleApp.regularFont, StyleApp.font12, { flex: 0.6, flexWrap: 'wrap' }]}>Specify the total number of Robins</Text>
                            <TextInput
                                blurOnSubmit
                                returnKeyType='done'
                                style={[AppStyle.boldFamily, AppStyle.text20, { borderBottomWidth: 2, width: 60, textAlign: 'center' }]}
                        ...
                        {this.state.formErrors.total_robins && <Text style={[AppStyle.errorText]}> {this.state.formErrors.total_robins}</Text>}


                        <View style={{ flex: 1, marginVertical: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Text style={[StyleApp.regularFont, StyleApp.font12, { flex: 0.6, flexWrap: 'wrap' }]}>Specify the total number of served people</Text>
                            <TextInput
                                blurOnSubmit
                                returnKeyType='done'
                                style={[AppStyle.boldFamily, AppStyle.text20, { borderBottomWidth: 2, width: 60, textAlign: 'center' }]}
                        ...
                        {this.state.formErrors.count_serve && <Text style={[AppStyle.errorText]}> {this.state.formErrors.count_serve}</Text>}

                        <View style={{ flex: 1, marginVertical: 10, }}>
                            <Text style={[StyleApp.regularFont, StyleApp.font12, { flex: 0.6, flexWrap: 'wrap' }]}>Upload Drive Images</Text>

                            <View style={{ borderRadius: 4, borderWidth: 0.5, minHeight: 200, marginTop: 10 }}>
                                <FlatList
                                ...
                                <TouchableOpacity style={{ marginVertical: 5 }} onPress={this.getImage}>
                                    <View style={{ flex: 1, alignItems: 'center', padding: 20, justifyContent: 'center' }}>
                                        ...
                                        <Text style={[AppStyle.text12, { textAlign: 'center', flexWrap: 'wrap' }]}>Upload minimum 4 Photos of the revent drive which reflects food distribution.</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                            {this.state.formErrors.album_images && <Text style={[AppStyle.errorText]}> {this.state.formErrors.album_images}</Text>}

                        </View>



                        <Text style={[AppStyle.text14, StyleApp.grayColor, StyleApp.mediumFont]}>Write short Description</Text>
                        ...
                        <TouchableOpacity style={[{ marginTop: 20, height: 44, paddingHorizontal: 30, borderRadius: 22, alignItems: 'center', justifyContent: 'center' }, AppStyle.primaryBackgroundColor]}
                            onPress={() => { this.submit() }}>
                            <Text style={{ color: '#fff', textTransform: 'uppercase', fontFamily: 'Montserrat-Bold' }}>Submit</Text>
                        </TouchableOpacity>
                ...
                {this.state.showComplete && <CompleteDriveModal showModal={this.state.showComplete} onClose={() => { this.setState({ showComplete: false }) }} navigation={this.props.navigation} />
                }
            </ScrollView>
        );
    }
