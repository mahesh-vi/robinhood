...
export default class UpcomingDriveDetail extends Component {
  ...
  joinDrive(reasonId) {
    ...
    DriveService.joinDrive(driveId, reasonId)
      .then((res) => {
        console.log(res);
  ...
  acceptDrive() {
    ...
    Userservice.acceptDrive(driveId)
      .then((res) => {
        console.log(res);
  ...
  rejectDrive() {
    ...
    Userservice.rejectDrive(driveId)
      .then((res) => {
        console.log(res);
  ...
  startDrive = () => {
    this.setState({loading: true});
    let driveId = this.driveParam.drive_id || this.driveParam.id;
    Userservice.startDrive(driveId)
      .then((res) => {
        console.log(res);
        this.props.screenProps.eventEmitter.emit('ViewUpdate', {count: 1});
        this.setState({loading: false});
  ...
  editDrive = () => {
    ...
    var merged = [
      ...driveDetail.invite_peoples,
      ...driveDetail.robins.filter((d) => !ids.has(d.id)),
    ];
    ...
    let data = {
      ...
      poc:
        driveDetail.admin.length > 0
          ? driveDetail.admin[0].admin_id
          : undefined,
        ...
        <View style={{marginHorizontal: 20, marginVertical: 15}}>
          <View style={{flexDirection: 'row'}}>
            ...
            <View style={{marginLeft: 30}}>
              <Text style={[StyleApp.headerText]}>
                {header ? header.title : 'Upcoming Drives'}
              </Text>
              <Text style={[StyleApp.headerSubText]}>
                {header
                  ? header.subTitle
                  : 'Enroll to participate in upcoming drives'}
              </Text>
            </View>
          </View>
          ...
          {this.state.drive && (
            <DriveDetailView
              ...
              joinDrive={this.joinDrive}
              accept={this.acceptDrive}
              reject={this.rejectDrive}
              edit={this.editDrive}
