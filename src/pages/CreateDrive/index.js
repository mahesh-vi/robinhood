...
export default class CreateDrive extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      .then((res) => {
        ...
        _this.setState(
          {inviteList: inviteList, foodTypes: foodTypes},
          () => {},
        );
      })
      .catch((error) => {
  ...
  removeRobin = (robin) => {
    ...
    inviteList.map((item) => {
      if (item.id == robin.id) return (item.selected = false);
    });
  ...
  validateField() {
    ...
    if (!numberOfVolunteers) {
      ...
      fieldValidationErrors.numberOfVolunteers =
        'Volunteers not be less than 3.';
    }
  ...
  render() {
    const {foodTypes, selectedInvites, inviteList, donerList, image_uri} =
      this.state;
      ...
      <ScrollView style={styles.mainContainer}>
        ...
        <View
          ...
          <View style={{marginLeft: 30}}>
            ...
            <Text style={[StyleApp.headerSubText]}>
              Create the Food Donation Drive
            </Text>
          </View>
        </View>
        ...
        <View style={{margin: 20, flexDirection: 'row'}}>
          <View style={{flex: 1}}>
            ...
            />
            {this.state.formErrors.foodQuantity && (
              <Text style={[AppStyle.errorText]}>
                {this.state.formErrors.foodQuantity}
              </Text>
            )}
          ...
          <View style={{flex: 1, marginLeft: 5}}>
            ...
            />
            {this.state.formErrors.numberOfVolunteers && (
              <Text style={[AppStyle.errorText]}>
                {this.state.formErrors.numberOfVolunteers}
              </Text>
            )}
