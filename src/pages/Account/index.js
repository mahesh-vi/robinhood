...
const UserType = {
    doner: "Doner", volunteer: "Volunteer", zone_admin: "Zone Admin", city_admin: "City Admin", state_admin: "State Admin"
...
export default class Account extends Component {
    ...
    UNSAFE_componentWillMount = () => {
        ...
        this.props.navigation.addListener("didFocus", async () => {
            ...
            Promise.all([
                ...
                global.user.type == "city_admin" && CityAdminService.getPendingConfirmationDrive(),
                global.user.type=="zone_admin"&& CityAdminService.getPendingConfirmationDrive({zone:userDetail.zone.id})
            ]).then((res) => {
                console.log(res);
                this.setState({
                    loading: false,
    ...
    renderItem = (item) => {
        ...
        const privousDrive = item.index > 0 ? (selectedIndex == 0 ? this.state.drives.data : selectedIndex == 1 ? this.state.requestedDrives.data : this.state.confirmationDrive.data)[item.index - 1] : item.item;
        ...
        if (selectedIndex == 0) {
            ...
            return (
                <DriveListItem isNotSameMonth={isNotSameMonth} drive={drive} removeCounter={true} onPress={() => {
    ...
    render() {
        ...
        const imageURL = user.image ? { uri: user.image } : require(`../../assets/images/user_male.png`);
        const { selectedIndex } = this.state;
        const tabOptions = this.state.userDetail.type == "city_admin" || this.state.userDetail.type=="zone_admin" ? ["My Drive", "Requested", "Confirmation"] : ["My Drive", "Requested"];
        return (
            <ScrollView style={styles.mainContainer}>
                ...
                <View style={{ marginHorizontal: 20 }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginVertical: 10 }}>
                        ...
                        <TouchableOpacity style={[{ borderRadius: 22, width: 50, height: 25, alignItems: 'center', justifyContent: 'center' }, AppStyle.primaryBackgroundColor]}
                            onPress={() => {
                                this.props.navigation.navigate('EditProfile');
                            }}  >
                            <Text style={{ color: '#fff', textTransform: 'uppercase', fontFamily: 'Montserrat-SemiBold', fontSize: 10 }}>Edit</Text>
                        </TouchableOpacity>

                    </View >

                    <View style={{ flex: 1, flexDirection: "row" }}>
                        <Image source={imageURL} style={[{ width: 120, height: 120, borderRadius: 60, borderWidth: 1 }, AppStyle.primaryBorder]} />
                        ...
                        <View style={{ flex: 1, marginLeft: 20, marginTop: 10 }}>
                            <Text style={[StyleApp.extraBoldFont, AppStyle.text18, AppStyle.darkGrayColor, { color: '#979797' }]}>{user.firstname + " " + user.lastname}</Text>
                            <Text style={[AppStyle.text12, AppStyle.grayColor, StyleApp.mediumFont]}>{UserType[user.type]}</Text>
                            <Text style={[{ marginTop: 5 }, AppStyle.blackColor, AppStyle.text12, StyleApp.mediumFont]}>{user.city.name}, {user.state.name}</Text>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>

                                {(this.state.achivements && this.state.achivements.bach_image) && this.state.achivements.bach_image.map((image, index) => {
                                    return (<Image key={index} source={{ uri: image }} style={[{ height: 35, width: 35 }]} resizeMode={'stretch'} />)
                                })
                                }
                    ...
                    <SegmentedControlTab
                        ...
                        tabsContainerStyle={{ marginTop: 10, borderWidth: 0, backgroundColor: '#fff' }}
                        tabStyle={{ backgroundColor: '#fff', borderWidth: 0, paddingBottom: 5, flexWrap: 'wrap', borderColor: '#fff' }}
                        activeTabStyle={{ backgroundColor: '#fff', borderColor: '#fff' }}
                        tabTextStyle={[AppStyle.semiBoldFamily, { color: '#77869E', paddingBottom: 5, borderBottomWidth: 2, borderBottomColor: '#fff' }]}
                        activeTabTextStyle={[AppStyle.semiBoldFamily, { color: '#00CA9D', borderBottomWidth: 2, borderBottomColor: StyleApp.primaryColor.color, }]}
                    />

                    <FlatList
                        data={CommonUtil.sortBy((selectedIndex == 0 ? this.state.drives.data : (selectedIndex == 1 ? this.state.requestedDrives.data : this.state.confirmationDrive.data)), {
                            prop: "date",
                            desc: true,
