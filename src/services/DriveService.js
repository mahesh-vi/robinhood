...
class DriveService {
    ...
    createFormData = (photo, body) => {
        ...
        if (Array.isArray(photo)) {
            ...
            photo.map((image) => {
                ...
                data.append('album_images[]', {
                    name: image.fileName || (Math.random().toString(36).substring(7) + "test.jpg"),
                    type: image.type|| image.mime,
                    uri:image.path
                }, image.fileName || Math.random().toString(36).substring(7)+"test.jpg");
    ...
    getOverallSnapshop(zoneId, duration) {
        const url = Constant.overallSnapShorURL + "/" + zoneId + "/" + global.user.id + "/" + duration;
        return (new Promise((resolve, reject) => {
    ...
    getDriveListOnZone(zoneId, duration) {
        const url = Constant.driveListOnZoneURL + "/" + zoneId + "/" + global.user.id + "/" + duration;
        return (new Promise((resolve, reject) => {
    ...
    completeDrive(driveId,driveDetail) {
        const url = Constant.completeCurrentDriveURL +  "/"+global.user.id+"/"+ driveId;
    ...
    getNextFoodDonationDrive(filterOption) {
        const url = Contstant.nextFoodDonationDriveURL;// + "/" + global.user.city_id;
        let requestData = {
            city_id: global.user.city_id,
